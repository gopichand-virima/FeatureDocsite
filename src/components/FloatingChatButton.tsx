      ...
      <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-3">
          ...
          <div className="relative animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="bg-slate-900 text-white px-4 py-3 rounded-lg shadow-lg max-w-xs">
              <div className="flex items-start gap-2 mb-2">
                <Sparkles className="h-4 w-4 mt-0.5 flex-shrink-0" style={{ color: '#7874F2' }} />
                <div>
        ...
        {conversationCount > 0 && (
          <button
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.borderColor = '#7874F2';
              const icon = e.currentTarget.querySelector('svg');
              const span = e.currentTarget.querySelector('span');
              if (icon) icon.style.color = '#7874F2';
              if (span) span.style.color = '#7874F2';
            }}
            onMouseLeave={(e) => {
        ...
        <Button
          ...
          style={{
            background: "linear-gradient(to right, #7874F2, #6B67E8)",
            boxShadow:
              "0 10px 25px -5px rgba(120, 116, 242, 0.3), 0 8px 10px -6px rgba(120, 116, 242, 0.3)",
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.background = "linear-gradient(to right, #6B67E8, #5F5BD9)";
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.background = "linear-gradient(to right, #7874F2, #6B67E8)";
          }}
        >
          ...
          <div className="absolute inset-[12.5%] rounded-full opacity-25 group-hover:opacity-40 transition-opacity duration-700 animate-[ping_1.5s_cubic-bezier(0.4,0,0.6,1)_infinite]" style={{
            background: "linear-gradient(to bottom right, rgba(120, 116, 242, 0.6), rgba(107, 103, 232, 0.6), rgba(95, 91, 217, 0.6))"
          }}></div>
        </Button>
      </div>
