...
A [client](/admin_discovery/client_ng.mdx) needs to be created to associate a client with an Infoblox credential.
...
Select if the scan should be *Immediate* or *Scheduled*. If *Scheduled* is selected, configure the *[Scan Frequency](/discovery/scans_frequency_ng.mdx)*.
