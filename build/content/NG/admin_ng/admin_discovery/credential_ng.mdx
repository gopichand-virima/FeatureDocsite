# Credentials

Use this function to create credentials that the Discovery Application uses to access remote hosts and run commands.

The discovery process requires authentication for conducting Active Directory scans, WMI scans, SNMP scans, SSH scans and other protocol-based scans. The corresponding credential information must be created and selected when setting up the appropriate scan. The credentials are stored in encrypted form in the Virima database.

In the main window, click **Admin > Discovery > Credentials**. The Credentials window displays.

## New Credential

From the *Select Actions* drop-down list, choose **New Credential**. The New Credential window displays.

Complete the required fields.

When all selections/entries are made, click **Add**.

## Edit Credential

Not all functions include an option to edit record line items shown in the main window.

Navigate to the window containing the record to edit.

Click the line containing the record. The applicable window or dialog box displays.

Make the necessary modifications.

Click **Save** or **Add**, as applicable.

When editing an existing item, other options may be available, such as adding [Tasks](/itsm/release_mngmt/tasks_ng.mdx), [Comments](/discovery/comments_ng.mdx), and [Attachments](/discovery/attachments_ng.mdx) and viewing [History](/common_topics/history_ng.mdx).

The options change based upon the function; therefore, not every option may be shown.

## Delete Credential

Deleting is a permanent action and cannot be undone. Deleting may affect other functionality and information in the application such as data in configured reports, fields in windows, selectable options, etc. Therefore, be sure to understand the potential effects before making a deletion.

Click the line that contains the item to delete.

From the *Select Actions* drop-down list, choose **Delete**. If a confirmation message is displayed, take the appropriate action (for example, click **OK** or **Continue**).

## Other Functions and Page Elements

- [Auto Refresh](/common_topics/auto_refresh_ng.mdx)
- [Delete](/common_topics/delete_remove_ng.mdx)
- [Go To Page](/common_functions/go_to_page_ng.mdx)
- [Personalize Columns](/common_topics/personalize_columns_ng.mdx)
- [Records per Page](/common_topics/records_per_page_ng.mdx)
- [Saved Filters](/common_topics/saved_filters_ng.mdx)
