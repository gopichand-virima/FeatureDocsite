 ...
 * 
 * Example: /admin_org_details/cost_center_6_1.mdx → /6.1/admin/organizational-details/cost-center
 * Example: /itsm/release_mngmt/tasks_6_1.mdx → /6.1/itsm/release-management/tasks
 * Example: /discovery/comments_6_1.mdx → /6.1/discovery-scan/.../comments
 */
...
async function resolveRouteFromTOC(
  ...
  try {
    // Construct the full file path from relative path
    const versionPath = version === 'NextGen' ? 'NG' : version.replace(/\./g, '_');
    
    // Try to extract the actual file path from the relative path
    // Patterns like /itsm/release_mngmt/tasks_6_1.mdx
    // or /admin_org_details/cost_center_6_1.mdx
    const cleanPath = relativePath.replace(/^\//, '').replace(/\.mdx$/, '');
        ...
        const findPage = (pages: any[]): any | null => {
          for (const page of pages) {
            ...
            if (page.filePath) {
              ...
              const fileName = filePathParts[filePathParts.length - 1]?.replace('.mdx', '');
 ...
 * 
 * @param relativePath - Relative path like /admin_org_details/cost_center_6_1.mdx or /itsm/release_mngmt/tasks_6_1.mdx
 * @param currentFilePath - Current file path for context (e.g., /content/6_1/admin_6_1/admin_org_details/about_org_details_6_1.mdx)
...
export async function convertMDXPathToRoute(relativePath: string, currentFilePath?: string): Promise<string | null> {
  if (!relativePath || !relativePath.endsWith('.mdx')) {
    return null;
  }
...
/**
 * Check if a path is a relative MDX file path that needs conversion
 */
