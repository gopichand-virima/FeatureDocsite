...
| **Domain** | The domain name managed by the Active Directory server (e.g., local.virima.com). |
...
| **Login Attribute** | The LDAP attribute used for user login (e.g.,userPrincipalName). userPrincipalName (UPN) is the default unique identifier for user logins. |
