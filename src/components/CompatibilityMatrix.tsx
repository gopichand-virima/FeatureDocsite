...
export function CompatibilityMatrix({
  ...
  useEffect(() => {
    const loadContent = async () => {
      ...
      try {
        ...
        const filePath = "/content/NG/compatibility_matrix/compatibility-matrix.mdx";
        const matrixContent = await getContent(filePath);
        if (matrixContent) {
          setContent(matrixContent);
        } else {
          console.error("Failed to load compatibility matrix content");
        }
      } catch (error) {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = "virima-compatibility-matrix.md";
    a.click();
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-slate-900">
            <article className="prose prose-slate dark:prose-invert max-w-none">
              <MDXRenderer 
                content={content} 
                filePath="/content/NG/compatibility_matrix/compatibility-matrix.mdx"
              />
            </article>
