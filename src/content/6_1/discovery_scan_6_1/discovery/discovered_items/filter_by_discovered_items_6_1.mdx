---
title: Filter By
description: >-
  Click the drop-down list and select one of the following: **Show All**,
  **New**, or **Update.** The record list is updated and displays the records
  that were scanned, discovered, or imported but are not present inside the
  CMDB.
canonical: >-
  https://docs.virima.com/6_1/discovery_scan_6_1/discovery/discovered_items/filter_by_discovered_items_6_1
keywords:
  - Filter By
  - Filter
  - By
  - Virima
lastUpdated: '2025-11-18'
---
# Filter By

Click the drop-down list and select one of the following: **Show All**, **New**, or **Update.** The record list is updated and displays the records that were scanned, discovered, or imported but are not present inside the CMDB.

- Default: Show All

<!-- -->

- Available options:

  - Show All: Displays both New and previously discovered items

  - New: Items not present in the CMDB.

  - Update: Items already in CMDB with new changes.

## Apply and Save a Filter

1.  Under the column header, click in the **Filter** field.

2.  Type your search text and press **Enter**. The list refreshes to show matching records.

3.  Click **Save Filter**.

4.  In the dialog, enter a name for the filter and click **Save**.

    <span class="image placeholder" original-image-src="../../Resources/Images/Common%20Topics/save-filter-dialog-box.PNG" original-image-title=""></span>

You can repeat these steps to create multiple saved filters.

## Apply or Delete a Saved Filter

1.  Click the **Saved Filter** icon on the toolbar to open the Filters list.

2.  Select a filter to apply it to your data.

3.  (Optional) Click **Set as Default** to make it load automatically.

4.  To remove a saved filter, click its **Delete** icon.

    <span class="image placeholder" original-image-src="../../Resources/Images/Common%20Topics/saved-filter-list.PNG" original-image-title=""></span>
